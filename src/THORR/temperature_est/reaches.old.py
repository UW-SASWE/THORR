...
def estimate_temperature(
    ...
    dels,  # path to dels csv file
    sarea,  # path to sarea csv file
):
    query = f"""
    SELECT 
    ...
    dels = pd.read_csv(dels)
    dels["Date"] = pd.to_datetime(dels["Date"])
    sarea = pd.read_csv(sarea)
    ...
    df.rename(
        columns={"dS (m3)": "dels", "area (km2)": "sarea", "Within_n*10km": "rel_dist"},
        inplace=True,
    )
    ...
    with open(model_var2, "rb") as f:
        model_var2_trained = pickle.load(f)

    df2 = df[(df["rel_dist"] != 0)].copy()
    X2 = df2[
        [
            "DayOfMonth_scaled",
            "Month_scaled",
    ...
    df2["est2"] = model_var2_trained.predict(X2)
...
def main(args):
    ...
    estimate_temperature(
        ...
        dels=Path(project_dir, config_dict["data"]["dels"]),
        sarea=Path(project_dir, config_dict["data"]["sarea"]),
    )
